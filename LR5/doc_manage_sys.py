...
#Создание доклада
@app.post("/reports", response_model=ReportResponse, tags=["Reports"], summary = "Create report")
...
@app.get("/reports", response_model=List[ReportResponse], tags=["Reports"], summary = "Get all reports")
def get_all_reports(db: Session = Depends(get_db)):
    return db.query(Report).all()

#Получение доклада по id
@app.get("/reports/{report_id}", response_model=ReportResponse, tags=["Reports"], summary = "Find report by id")
...
@app.get("/reports", response_model=List[ReportResponse], tags=["Reports"], summary = "Get all reports in the conference.")
def get_report(report_status: str = "accepted", db: Session = Depends(get_db)):
    return db.query(Report).filter(Report.status == report_status).all()

#Добавление доклада в конференцию
@app.post("/reports/{report_id}/{status}", response_model=ReportResponse, tags=["Reports"], summary = "Add report to the conference")
def get_report(report_id: int, db: Session = Depends(get_db)):
