...
# GET /users/login/{login} - Получить пользователя по логину (требует аутентификации)
@app.get("/users/login/{login}", response_model=User)
...
# GET /users/mask/{mask} - Получить пользователя по маске имя+фамилия (требует аутентификации)
@app.get("/users/mask/{mask}", response_model=List[User])
def get_mask(mask: str, current_user: str = Depends(get_current_client)):
    mask = mask.lower().replace(" ", "") 
    print(mask)

    results = [
        user for user in users_db
        if mask in (user.first_name + user.last_name).lower()
    ]
    if not results:
